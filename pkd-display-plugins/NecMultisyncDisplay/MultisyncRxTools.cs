namespace NecMultisync
{
	...
	internal static class MultisyncRxTools
	{
		...
		/// 3 - Display Port,
		/// </summary>
		public static readonly List<byte[]> InputSelectCommands = new List<byte[]>()
		{
			new byte[] {},
			new byte[] { 0x01, 0x30, 0x41, 0x30, 0x45, 0x30, 0x41, 0x02, 0x30, 0x30, 0x36, 0x30, 0x30, 0x30, 0x31, 0x31, 0x03, 0x72, 0x0D },
			new byte[] { 0x01, 0x30, 0x41, 0x30, 0x45, 0x30, 0x41, 0x02, 0x30, 0x30, 0x36, 0x30, 0x30, 0x30, 0x31, 0x32, 0x03, 0x71, 0x0D },
			new byte[] { 0x01, 0x30, 0x41, 0x30, 0x45, 0x30, 0x41, 0x02, 0x30, 0x30, 0x36, 0x30, 0x30, 0x30, 0x30, 0x46, 0x03, 0x04, 0x0D },
		};

		public static readonly Dictionary<InputRxTypes, byte[]> InputRxData = new Dictionary<InputRxTypes, byte[]>()
		{
			{ InputRxTypes.QueryHdmi1, new byte[] {0x31, 0x31, 0x03, 0x01} },
			{ InputRxTypes.QueryHdmi2, new byte[] {0x31, 0x32, 0x03, 0x02} },
			{ InputRxTypes.QueryDp, new byte[] {0x30, 0x46, 0x03, 0x77} },
			{ InputRxTypes.AckHdmi1, new byte[] {0x31, 0x31, 0x03, 0x03} },
			{ InputRxTypes.AckHdmi2, new byte[] {0x31, 0x32, 0x03, 0x00} },
			{ InputRxTypes.AckDp, new byte[] {0x30, 0x46, 0x03, 0x75} },
		};

		/// <summary>
		/// Compare 2 byte arrays for equivalence. they will be consitered equivalent if each byte in
